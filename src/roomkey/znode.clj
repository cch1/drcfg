                     ...
                     (when-let [cwms (zclient/with-connection handle-connection-loss
                                       (case event-type
                                         ...
                                         :NodeCreated (do
                                                        ...
                                                        (let [cwms (reduce (fn [cwms child]
                                                                             (assert (nil? (cwms child)) "Child wants to be watched twice!")
                                                                             ;; watch must be in place before create to generate proper events
                                                                             (assoc cwms child (watch child)))
                                                                           cwms @children)]
                                                          (async/>! znode-events {:event-type :NodeDataChanged})
                                                          (async/>! znode-events {:event-type :NodeChildrenChanged})
                                                          cwms))
                                         :NodeDeleted (do (log/debugf "Node %s deleted" (str this)) ; Event is generated by exists, data and child watches.
                                                          (async/>! delete-events {::type ::deleted!})
                                                          cwms)
                                         :NodeDataChanged (do
                                                            (async/>!! data-events (log/spy :trace (zclient/data client path {:watcher watcher})))
                                                            cwms)
