...
(fact "Can open and close a ZNode"
      (let [$root (new-root)
            alm (open $root (str connect-string sandbox))]
        alm => (partial instance? java.lang.AutoCloseable)
        (Thread/sleep 100) ; Let the watch ops complete to avoid exceptions trying to use a closed client
        (.close alm) => nil?))

(fact "Root ZNode streams exists data"
      (let [$root (new-root)]
        (with-open [_ (open $root (str connect-string sandbox))]
          $root => (eventually-streams 1 1000 (just [(just #::znode{:type ::znode/exists :stat (stat? {:version 0})})])))
...
(fact "ZNodes can be actualized and stream current value"
      (let [$root (new-root)
        ...
        (with-open [_ (open $root (str connect-string sandbox))]
          $root => (eventually-streams 1 1000 (just [(just #::znode{:type ::znode/exists :stat (stat? {:version 0})})]))
          $child => (eventually-streams 1 1000 (just [(just #::znode{:type ::znode/created! :stat (stat? {})})]))
          (Thread/sleep 100)) ; Let ref state settle with unstreamed child update.
...
(fact "Existing ZNodes are acquired and stream their current value"
      (let [$root (new-root)
        ...
        (with-open [_ (open $root (str connect-string sandbox))]
          $child => (eventually-streams 1 1000 (just [(just #::znode{:type ::znode/created! :stat (stat? {})})]))
          $grandchild => (eventually-streams 1 1000 (just [(just #::znode{:type ::znode/created! :stat (stat? {})})]))
          (Thread/sleep 100))
        ;; ensure spurious wrongly-pathed acquired children don't appear
        (let [$root (new-root)]
          (with-open [_ (open $root (str connect-string sandbox))]
            $root => (eventually-streams 2 2000 (just [(just #::znode{:type ::znode/exists :stat (stat? {:version 0})})
            ...
            (let [$child ($root "/child")]
              ...
              (let [$grandchild ($root "/child/grandchild")]
                ...
                $grandchild => (eventually-streams 2 2000 (just [(just #::znode{:type ::znode/exists :stat (stat? {:version 0})})
                                                                 (just #::znode{:type ::znode/sync-data :value 0 :stat (stat? {:version 0})})]))
                $child => (eventually-streams 1 1000 [::th/timeout]))
...
(fact "Existing ZNodes do not stream confirmed value at startup when local and remote values are equal"
      (let [$root (new-root)
            $child (add-descendant $root "/child" 0)]
        (with-open [_ (open $root (str connect-string sandbox))]
          $child => (eventually-streams 1 2000 (just [(just #::znode{:type ::znode/created! :stat (stat? {})})]))
          (update! $child 1 0 {})
          $child => (eventually-streams 1 2000 (just [(just #::znode{:type ::znode/data-changed :value 1 :stat (stat? {:version 1})})]))))

      (let [$root (new-root)
            $child (add-descendant $root "/child" 1)]
        (with-open [_ (open $root (str connect-string sandbox))]
          $child => (eventually-streams 1 1000 (just [(just #::znode{:type ::znode/exists :stat (stat? {:version 1})})]))
...
(fact "ZNodes stream pushed values"
      (let [$root0 (new-root)
        ...
        (with-open [_ (open $root0 (str connect-string sandbox))]
          $child0 => (eventually-streams 1 1000 (just [(just #::znode{:type ::znode/created! :stat (stat? {})})]))
          (with-open [_ (open $root1 (str connect-string sandbox))]
            $child1 => (eventually-streams 1 1000 (just [(just #::znode{:type ::znode/exists :stat (stat? {:version 0})})]))
            (update! $child0 1 0 {})
...
(fact "Existing ZNodes stream pushed values exactly once"
      (let [$root0 (new-root)
        ...
        (with-open [_ (open $root1 (str connect-string sandbox))]
          $child1 => (eventually-streams 1 1000 (just [(just #::znode{:type ::znode/exists :stat (stat? {:version 0})})]))
          $child1 => (eventually-streams 1 2000 [::th/timeout])
...
(fact "Root node behaves like a leaf"
      ...
      (let [$root (new-root "/" nil)]
        (with-open [_ (open $root (str connect-string sandbox))]
          $root => (eventually-streams 2 1000 (just [(just #::znode{:type ::znode/exists :stat (stat? {:version 1})})
...
(fact "Existing ZNodes can be deleted"
      (let [$root (new-root)
        ...
        (with-open [_ (open $root (str connect-string sandbox))]
          $child => (eventually-streams 1 1000 (just [(just #::znode{:type ::znode/created! :stat (stat? {})})])))
        (with-open [_ (open $root (str connect-string sandbox))]
          $child => (eventually-streams 1 1000 (just [(just #::znode{:type ::znode/exists :stat (stat? {:version 0})})]))
          (delete! $child 0 {}) => truthy
          $child => (eventually-streams 1 1000 (just #{#::znode{:type ::znode/deleted!}})))))
...
(fact "Children do not intefere with their parents, regardless of insertion sequence"
      (let [$root (new-root)
        ...
        (with-open [_ (open $root (str connect-string sandbox))]
          $zA => (eventually-streams 1 1000 (just [(just #::znode{:type ::znode/created! :stat (stat? {})})]))
          $zB => (eventually-streams 1 1000 (just [(just #::znode{:type ::znode/created! :stat (stat? {})})])))
        $zA => (eventually-streams 1 1000 [::th/timeout])
        $zB => (eventually-streams 1 1000 [::th/timeout])))
...
(fact "A ZNode can persist metadata"
      (let [$root (new-root)
        ...
        (with-open [c (zoo/connect (str connect-string sandbox))]
          (with-open [_ (open $root (str connect-string sandbox))]
            $z0 => (eventually-streams 1 1000 (just [(just #::znode{:type ::znode/created! :stat (stat? {})})]))
            $z1 => (eventually-streams 1 1000 (just [(just #::znode{:type ::znode/created! :stat (stat? {})})]))
            $z2 => (eventually-streams 1 1000 (just [(just #::znode{:type ::znode/created! :stat (stat? {})})]))
...
(fact "Children are remembered across sessions"
      (let [$root (new-root)]
        (with-open [c (zoo/connect (str connect-string sandbox))]
          (with-open [_ (open $root (str connect-string sandbox))]
            $root => (eventually-streams 1 3000 (just [(just #::znode{:type ::znode/exists :stat (stat? {})})]))
          ...
          (with-open [_ (open $root (str connect-string sandbox))]
            $root => (eventually-streams 1 1000 (just [(just #::znode{:type ::znode/exists :stat (stat? {:version 0})})]))
            (Thread/sleep 200))
          $root => (eventually-streams 1 1000 [::th/timeout]))))
...
(fact "Added descendant ZNodes can be created and will merge into watched tree"
      (let [$root (new-root)
            $child (add-descendant $root "/child" 0)]
        (with-open [_ (open $root (str connect-string sandbox))]
          $root => (eventually-streams 1 1000 (just [(just #::znode{:type ::znode/exists :stat (stat? {})})]))
          $child => (eventually-streams 1 1000 (just [(just #::znode{:type ::znode/created! :stat (stat? {})})]))
          (let [$grandchild (add-descendant $root "/child/grandchild" (with-meta #{1 2 3} {:foo "bar"}))]
            (create! $grandchild {}) => (stat? {})
            $grandchild => (eventually-streams 1 1000 (just [(just #::znode{:type ::znode/exists :stat (stat? {:version 0})})]))))
        (let [$grandchild ($root "/child/grandchild")]
