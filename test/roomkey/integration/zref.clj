...
(fact "Initializing a ZRef in a virgin zookeeper creates the node with default data"
  (with-open [$c (zoo/connect (str connect-string sandbox))]
    (.zInitialize (.zPair (zref "/myzref" "A") $c)) => truthy)
  (with-open [c (zoo/connect (str connect-string sandbox))]
...
(fact "Can update a connected ZRef"
  (with-open [$c (zoo/connect (str connect-string sandbox))]
    (let [$z (.zConnect (.zInitialize (.zPair (zref "/zref0" "A") $c)))]
      (.compareVersionAndSet $z 1 "B") => true
      (.compareVersionAndSet $z 12 "C") => false)
    (let [$z (.zConnect (.zInitialize (.zPair (zref "/zref1" "A") $c)))]
      (compare-and-set! $z "Z" "B") => false
      (compare-and-set! $z "A" "B") => true
      (reset! $z "C") => "C")
    (let [$z (.zConnect (.zInitialize (.zPair (zref "/zref2" 1) $c)))]
      (swap! $z inc) => 2
      (swap! $z inc) => 3
      (Thread/sleep 1000))))
...
(fact "A connected ZRef is updated by changes at the cluster"
  (with-open [$c (zoo/connect (str connect-string sandbox))]
    (let [$z (.zConnect (.zInitialize (.zPair (zref "/myzref" "A") $c)))
          sync (promise)]
...
(fact "A connected ZRef is not updated by invalid values at the cluster"
  (with-open [$c (zoo/connect (str connect-string sandbox))]
    (let [$z (.zConnect (.zInitialize (.zPair (zref "/myzref" "A" :validator string?) $c)))
          sync (promise)]
      (add-watch $z :sync (fn [& args] (deliver sync args)))
      (with-open [c (zoo/connect (str connect-string sandbox))]
...
(fact "Children do not intefere with their parents"
  (with-open [$c (zoo/connect (str connect-string sandbox))]
    (let [$zB (.zConnect (.zInitialize (.zPair (zref "/myzref/child" "B" :validator string?) $c)))
          $zA (.zConnect (.zInitialize (.zPair (zref "/myzref" "A" :validator string?) $c)))
          sync-a (promise)
          sync-b (promise)]
  ...
  (with-open [s (TestingServer. true)]
    (let [z0 (zref "/myzref" "A")
      ...
      (with-open [c0 (zoo/connect (.getConnectString s))]
        (.zConnect (.zInitialize (.zPair z0 c0)))
        (reset! z0 "B")
        (.zDisconnect z0))
      (with-open [c1 (zoo/connect (.getConnectString s))]
        (.zConnect (.zInitialize (.zPair z1 c1)))
        (reset! z1 "C")
        (reset! z1 "D")
        (.zConnect (.zInitialize (.zPair z0 c1)))
        z0)) => (refers-to "D")))
